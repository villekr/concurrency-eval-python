...
_S3_CONFIG = Config(max_pool_connections=64)
...
async def processor(event):
    ...
    async with _SESSION.create_client("s3", config=_S3_CONFIG) as s3:
...
async def get(s3, bucket_name: str, key: str, find: str, sem: asyncio.Semaphore) -> Optional[str]:
    async with sem:
        ...
        if find:
            ...
            if isinstance(find, str):
                ...
                # Just in case the caller provided bytes already
                find_bytes = bytes(find)
